...
app.py
======
전국 Car-BTI 대시보드 (16종 MBTI 1:1 맞춤형 연동)
...
[Tab 2] 나의 Car-BTI 테스트
  4문항 설문 → 본인 페르소나 산출
  → 가장 비슷한 지역 Top 3 / 매칭 차량 / 맞춤 FAQ Top 3 / 맞춤형 뉴스
"""
...
def load_region_stats():
    """MySQL `region_stats` 테이블에서 17개 시도 데이터 로드

    팀 저장소 스키마: region, eco_ratio, large_ratio,
    female_ratio, import_ratio, persona_code(4자리 [E/G][L/S][F/M][I/D]) 등
    ※ region_stats 는 팀 공유 SQL 덤프를 DBeaver 로 임포트해 적재함
    """
    try:
        engine = get_engine()
        ...
        st.info(
            "→ 팀 공유 SQL 덤프를 DBeaver 로 임포트해 region_stats 테이블을 먼저 적재해야함"
        )
        st.stop()

    if df.empty:
        st.error("❌ region_stats 가 비어 있습니다.")
        st.info("→ 팀 공유 SQL 덤프를 DBeaver 로 임포트해 region_stats 테이블을 적재해야함")
        st.stop()
...
@st.cache_data(ttl=300)
def load_db():
    """MySQL에서 FAQ + 추천 차량 로드. 실패 시 빈 DF 반환"""
    try:
        engine = get_engine()
    except Exception as exc:
...
def render_recommended_cars(persona: str, cars: pd.DataFrame):
    ...
    rec = get_recommended_cars(persona, cars)
    if rec.empty:
        st.warning(f"⚠️ [{persona}] 유형에 매칭된 차량이 없음")
        return
            ...
            with col1:
                ...
                if img_bytes:
                    ...
                    st.markdown(
                        ...
                        "🚗<br/><span style=\"font-size:11px\">이미지를 불러올 수 없음</span>"
                        "</div>",
...
def render_faq_list(
    ...
    if "company" in scored.columns and brands:
        ...
        # 점수가 높은 시드에 묻히지 않고 고르게 상위 노출되도록 함
