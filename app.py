...
# persona_cars, company_faq 는 다른 팀원이 SQLite 에 적재하고 있다고 가정 
SQLITE_PATH = "db/car_bti.db"

# MySQL region_stats 의 "서울" ↔ geojson 의 "서울특별시" 매핑
REGION_FULL_MAP = {
...
AXIS_DESC = {
    "E": "친환경 차량(전기·수소·하이브리드) 보급률이 전국 평균보다 높은 지역",
    "G": "내연기관(휘발유·경유·LPG) 비중이 높고 익숙함을 선호하는 지역",
    "L": "캠핑·레저·다목적 공간을 위한 SUV·대형차 선호도가 높은 지역",
...
VIZ_MODES = {
    ...
    "🌍 수입차 비율":        ("import_ratio", "Blues",   "수입차 비율 (%)"),
...
def load_region_stats():
    """MySQL `region_stats` 에서 17개 시도 4축 데이터를 로드."""
    try:
        ...
        st.info(
            "→ app.py 의 `MYSQL_CONFIG` 비밀번호를 확인하거나, "
            "`load_to_mysql.py` 를 먼저 실행했는지 확인하세요."
        )
        st.stop()
    df["region_full"] = df["region"].map(REGION_FULL_MAP)
    return df


@st.cache_data
def load_sqlite():
    """SQLite 에서 persona_cars, company_faq 로드 (다른 팀원 작업, 없어도 동작)."""
...
def make_radar(region: str, row) -> go.Figure:
    ...
    # 분포 범위: eco 10~25%, large 13~17%, female 23~33%, import 7~23%
...
def render_recommended_cars(persona: str, cars: pd.DataFrame):
    ...
    for i, axis in enumerate(persona):
        ...
        with cols[i]:
            ...
            if sub.empty:
                # F/M 자리는 persona_cars 에 아직 데이터 없음 → 추후 추가 예정
                st.info(
                    f"'{axis}' 축 추천 차량 데이터는 아직 등록되지 않았습니다.\n\n"
                    f"setup_db.py 의 persona_cars 에 axis='{axis}' 데이터를 "
                    f"추가하면 여기에 표시됩니다."
                )
                continue
...
def render_faq_list(persona: str, faq: pd.DataFrame, top_n=None):
    if faq.empty:
        st.warning("⚠️ FAQ 가 비어있습니다. `python crawler/crawl_faq.py` 를 실행해주세요.")
        return
    if "persona_tags" not in faq.columns:
        st.warning("⚠️ persona_tags 컬럼이 없습니다. setup_db.py / crawl_faq.py 를 다시 실행해주세요.")
...
def on_region_input():
    ...
    for r in df_stats["region"].values:
        if user_input[:2] in r or r in user_input or user_input[:2] in REGION_FULL_MAP.get(r, ""):
...
with st.expander("🧬 Car-BTI 가 처음이신가요? — 4가지 축 설명 보기", expanded=False):
    ...
    st.markdown(
        ...
        "📌 <b>예시</b> &nbsp;&nbsp; <code>ELFD</code> = 친환경(E) + 대형/SUV(L) + 여성 강세(F) + 국산(D) → "
        "<b>제주특별자치도 스타일</b> · 친환경 SUV와 국산차를 선호하는 여성 등록 비율이 높은 지역"
        "</div>",
        unsafe_allow_html=True,
    )
...
with tab1:
    ...
    with col_map:
        ...
        if map_data and map_data.get("last_active_drawing"):
            ...
            for r in df_stats["region"].values:
                if clicked.startswith(r[:2]) or r.startswith(clicked[:2]):
                    st.session_state.selected_region = r
                    break
    ...
    with col_info:
        st.subheader("🔍 지역 분석")
        st.text_input(
            "거주하시거나 궁금한 지역을 입력하세요 (예: 서울, 부산, 강원)",
            key="region_input",
        ...
        with st.expander("📊 차량 통계 요약", expanded=True):
